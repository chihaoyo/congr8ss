...
var MeetingID = function(term, sessionPeriod, sessionTimes, meetingTimes, name) {
  //console.log(name);
  this.term = parseInt(term);
  this.session = parseInt(sessionPeriod);

  var a = (sessionTimes == null || sessionTimes == 'null' ? 0 : parseInt(sessionTimes));
  var b = (meetingTimes == null || meetingTimes == 'null' ? 0 : parseInt(meetingTimes));
  if(b != 0) { //this.name.indexOf('臨時會') != -1) { // 是臨時會
    this.emergencyMeeting = a;
    this.meeting = b;
  }
  else {
    this.emergencyMeeting = b;
    this.meeting = a;
  }
  if(name != undefined) {
    this.isGeneralAssembly = (name.indexOf('全院委員') != -1);
    this.name = name;
  }
  ...
  this.fullID = pad(this.term) + '-' + pad(this.session) + '-[' + pad(this.emergencyMeeting) + ']-' + (this.isGeneralAssembly === true ? 'g' : '') + pad(this.meeting) + ':' + (this.name ? this.name : '?');
  this.numericID = pad(this.term) + '-' + pad(this.session) + '-' + pad(this.emergencyMeeting) + '-' + pad(this.meeting);
};
MeetingID.prototype.toString = function() {
  return this.fullID;
};

var Meeting = function(data) {
  this.id = new MeetingID(data.term, data.sessionPeriod, data.sessionTimes, data.meetingTimes, data.meetingName);
  this.name = data.meetingName; // duplicate
  var datetime = data.meetingDateDesc.split(' ');
  var date = datetime[0].split('/');
  this.date = '' + (parseInt(date[0]) + 1911) + '-' + date[1] + '-' + date[2];
  var time = datetime[1].split('-');
  this.time = {start: time[0], finish: time[1]};
};
